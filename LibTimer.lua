...
if not Library.LibTimer then Library.LibTimer = {} end

function Library.LibTimer.timer()
   -- the new instance
   local self =   {
...
--       print(string.format("LibTimer: timedeventsmanager(%s) watchdog(%s), timerstart(%s)", now, self.watchdog, self.timerstart))
      ...
      if self.watchdog == true then
         ...
         if self.timerstart == nil then
            ...
            if (now - self.timerstart) >= self.time2wait then
               ...
               -- remove handler if it's not a  recurring event
   ...
   function self.add(callback, time2wait, recurring)
      ...
      if callback and time2wait then
         ...
         print(string.format("LibTimer.addtimer ERROR: <callback=[%s]> <time2wait=[%s]> [<recurring=[%s]>]", callback, time2wait, recurring))
   ...
   function self.del(callback, time2wait, recurring)
      ...
      if callback and time2wait then
         ...
         print(string.format("LibTimer.deltimer ERROR: <callback=[%s]> <time2wait=[%s]> [<recurring=[%s]>]", callback, time2wait, recurring))
